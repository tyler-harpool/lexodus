...
pub async fn create_docket_entry(
    ...
    for sc in &report.status_changes {
        ...
        let _ = crate::repo::case::update_status(&pool, &court.0, entry.case_id, &sc.new_status).await;
    }

    // Start Speedy Trial clock if rules require it (best-effort)
    if report.start_speedy_trial {
        tracing::info!(case_id = %entry.case_id, "Compliance engine starting Speedy Trial clock");
...
pub async fn list_by_type(
    ...
    let rows = sqlx::query_as!(
        ...
        LEFT JOIN criminal_cases cc ON de.case_id = cc.id
        LEFT JOIN civil_cases cv ON de.case_id = cv.id
        WHERE de.court_id = $1 AND de.case_id = $2 AND de.entry_type = $3
        ORDER BY de.entry_number ASC
        "#,
        court.0,
        uuid,
...
pub async fn list_sealed(
    ...
    let rows = sqlx::query_as!(
        ...
        LEFT JOIN criminal_cases cc ON de.case_id = cc.id
        LEFT JOIN civil_cases cv ON de.case_id = cv.id
        WHERE de.court_id = $1 AND de.case_id = $2 AND de.is_sealed = true
        ORDER BY de.entry_number ASC
        "#,
        court.0,
        uuid,
...
pub async fn search_in_case(
    ...
    let rows = sqlx::query_as!(
        ...
        LEFT JOIN criminal_cases cc ON de.case_id = cc.id
        LEFT JOIN civil_cases cv ON de.case_id = cv.id
        WHERE de.court_id = $1 AND de.case_id = $2 AND de.description ILIKE $3
        ORDER BY de.entry_number ASC
        "#,
        court.0,
        uuid,
