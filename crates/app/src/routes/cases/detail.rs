use dioxus::prelude::*;
use shared_types::{
    CaseResponse, DocketAttachmentResponse, DocketEntryResponse, DocketSearchResponse,
...
use shared_ui::components::{
    ...
    DataTableCell, DataTableColumn, DataTableHeader, DataTableRow, DetailFooter, DetailGrid,
    DetailItem, DetailList, FormSelect, Input, PageActions, PageHeader, PageTitle, Separator,
    Skeleton, TabContent, TabList, TabTrigger, Tabs, Textarea, Tooltip, TooltipContent,
    TooltipTrigger,
};
            ...
            Card {
                ...
                CardContent {
                    DetailList {
                        ...
                        if let Some(ref judge_id) = case_item.assigned_judge_id {
                            DetailItem { label: "Assigned Judge",
                                Tooltip {
                                    TooltipTrigger { "{judge_id}" }
                                    TooltipContent { "Judge assigned to this case" }
                                }
                            }
                        }
...
#[component]
fn DocketTab(case_id: String) -> Element {
    let ctx = use_context::<CourtContext>();
    let court_id = ctx.court_id.read().clone();
    let cid = case_id.clone();
